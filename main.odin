...
update_state :: proc()
{
	...
	{ 
		//Swap turns, search for checks / checkmates 
		verify_requested_move_legal(mouse_rank, mouse_file)
	}

	if rl.IsMouseButtonPressed(.LEFT) == true 
	{
		verify_selected_own_piece(mouse_rank, mouse_file)
		set_legal_moves()
	}
...
verify_requested_move_legal :: proc(requested_rank: i32, requested_file: i32)
{
	piece := &selected_piece[.PIECE]
	old_rank := selected_piece[.RANK]
	old_file := selected_piece[.FILE]
	new_rank := i8(requested_rank)
	new_file := i8(requested_file)
...
verify_selected_own_piece :: proc(mouse_rank: i32, mouse_file: i32)
{
	current_piece := &board_state[mouse_rank][mouse_file] 
...
search_for_checks :: proc(board: ^[8][8]i8)
{
	for r in 0..<8
	{
		for f in 0..<8
		{
			...
			case .BLACK_ROOK..=.BLACK_PAWN:
				if turn == .WHITE
				{
					search_legal_moves_from_piece(i8(r), i8(f), Piece(board_state[r][f]), board)
					//If checked, white_checked = true
				}
...
set_legal_moves :: proc()
{
	...
	search_legal_moves_from_piece(rank, file, piece, &board_state)
}

verify_simulation_removes_check :: proc(old_rank, 
					old_file, 
					new_rank, 
					new_file: i8, 
					piece: Piece)
{
	sim_board_state = board_state
	sim_board_state[old_rank][old_file] = i8(Piece.NONE)
	sim_board_state[new_rank][new_file] = i8(piece)
	search_for_checks(&sim_board_state) 
}

search_legal_moves_from_piece :: proc(rank, file: i8, piece: Piece, board: ^[8][8]i8)
{
	//Reset legal moves
	for rank in 0..<8
	{
	...
	case .WHITE_PAWN, .BLACK_PAWN: 
		...
		{
			// Normal capture
			if board[rank + dir][file + 1] != i8(Piece.NONE) 
			{
		...
		if board[rank + dir][file] == i8(Piece.NONE)
		{
		...
		{
			if board[rank + (2 * dir)][file] == i8(Piece.NONE) 
			{
	...
	// All other pieces
	case .WHITE_ROOK..=.WHITE_KING, .BLACK_ROOK..=.BLACK_KING: 
		for direction in 0..<8
		{
			...
			for hit_invalid_square == false 
			{
				...
				if new_rank >= 0  &&
					...
					new_file <= 7 
				{
					search_square := Piece(board[new_rank][new_file])
