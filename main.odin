...
check_piece_placement_legal :: proc(piece: u8,
				old_rank: u8,
				old_file: u8,
				new_rank: u8,
				new_file: u8)
...
insert_piece_into_board_state :: proc(piece: rune, rank: int, file: int, from_fen: bool)
{
	...
	{
	case 'r':
		board_state[local_rank][file] = u8(Piece.BLACK_ROOK)
	case 'n':
		board_state[local_rank][file] = u8(Piece.BLACK_KNIGHT)
	case 'b':
		board_state[local_rank][file] = u8(Piece.BLACK_BISHOP)
	case 'k':
		board_state[local_rank][file] = u8(Piece.BLACK_KING)
	case 'q':
		board_state[local_rank][file] = u8(Piece.BLACK_QUEEN)
	case 'p':
		board_state[local_rank][file] = u8(Piece.BLACK_PAWN)
	case 'R':
		board_state[local_rank][file] = u8(Piece.WHITE_ROOK)
	case 'N':
		board_state[local_rank][file] = u8(Piece.WHITE_KNIGHT)
	case 'B':
		board_state[local_rank][file] = u8(Piece.WHITE_BISHOP)
	case 'K':
		board_state[local_rank][file] = u8(Piece.WHITE_KING)
	case 'Q':
		board_state[local_rank][file] = u8(Piece.WHITE_QUEEN)
	case 'P':
		board_state[local_rank][file] = u8(Piece.WHITE_PAWN)
	}
}
...
get_texture :: proc(piece: u8) -> rl.Texture 
{
